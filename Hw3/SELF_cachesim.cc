...
void cache_sim_t::init()
{
  ...
  lru = new uint64_t[sets*ways](); //pao
...
uint64_t* cache_sim_t::check_tag(uint64_t addr)
{
  ...
  for (size_t i = 0; i < ways; i++) //檢查某index中每個block
    if (tag == (tags[idx*ways + i] & ~DIRTY)){  //hit
      ...
      if (lru[idx*ways+i] != ways){ //如果hit本來就最新則不用改
      for (size_t j=0; j<ways; j++){  //更新該set的lru
          lru[idx*ways+i] = ways;
          if (lru[idx*ways+j]!=0 && j!=i)  lru[idx*ways+j]--;         
        }
      }
...
uint64_t cache_sim_t::victimize(uint64_t addr)
{
  ...
  size_t way = 0;
  size_t lru_min = lru[idx*ways]; //取出該set第一個block的lru值
  size_t lfu_min = lfu[idx*ways]; //取出該set第一個block的lfu值
  for (size_t i=1; i<ways; i++){  
    if (lfu[idx*ways+i] < lfu_min){ //找出最小lfu
      way = i;
      lfu_min = lfu[idx*ways+i];
      lru_min = lru[idx*ways+i];
    }
    if(lfu[idx*ways+i] = lfu_min){  //找出同為最小lfu中最小的lru
      if (lru[idx*ways+i] < lru_min){
      way = i;
      lru_min = lru[idx*ways+i];
      }
    }
  ...
  for (size_t i=0; i<ways; i++){  //更新該set的lru
    lru[idx*ways+way] = ways;
    //lru會在[1-ways]區間，0代表還沒用過，但現在選擇的lru不一定是最小值了，所以把大於所選的那些block更新就好，才能維持[1-ways]區間
    if (lru[idx*ways+i]!=0 && i!=way && lru[idx*ways+i]>lru_min)  lru[idx*ways+i]--;    
  }
