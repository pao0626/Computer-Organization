...
void cache_sim_t::init()
{
  ...
  fifo_indexs = new size_t[sets](); //pao
  cnt = 0;  //pao
  read_accesses = 0;
  read_misses = 0;
  bytes_read = 0;
...
uint64_t cache_sim_t::victimize(uint64_t addr)
{
  size_t idx = (addr >> idx_shift) & (sets-1);
  //pao
  uint64_t victim = tags[idx*ways + fifo_indexs[idx]];
  if(cnt<10){ //看前10筆
    size_t input = (addr >> idx_shift) | VALID;
    std::cout << "input data:" << input << std::endl;
    for(size_t i=0; i<sets*ways; i++){
      std::cout << "cache now is :" << tags[i] << std::endl;
    }
    std::cout << "set:" << idx << " and way:" << fifo_indexs[idx] <<std::endl;
    std::cout << "find victim:" << victim << std::endl;
  }
  tags[idx*ways + fifo_indexs[idx]] = (addr >> idx_shift) | VALID;
  fifo_indexs[idx] = (fifo_indexs[idx]+1) % ways;
  cnt++;
  return victim;
}
